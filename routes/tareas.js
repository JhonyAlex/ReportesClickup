 ...
 * @param {import('express').Request} req
 * @returns {{spaceId: string, token: string, params: Record<string, any>}|null}
 */
function obtenerParametros(req) {
  const { space_id: spaceId, token: _unused, dias, ...rest } = req.query;
  ...
  return { spaceId, token, params };
}

/**
 * Maneja la obtenci\xC3\xB3n de tareas desde ClickUp.
 * Requiere el par\xC3\xA1metro `space_id` y un token v\xC3\xA1lido en la configuraci\xC3\xB3n
 * o en la consulta.
 */
async function manejarApiTareas(req, res) {
  const data = obtenerParametros(req);
  if (!data) {
    return res.status(400).json({ error: 'Par\xC3\xA1metro space_id o token faltante' });
  }

  const { spaceId, token, params } = data;

  try {
    const datos = await obtenerTareas(spaceId, token, params);
    res.json(datos);
  } catch (err) {
    if (err.status === 404) {
      return res.status(404).json({
        error: 'Space ID inv\xC3\xA1lido o sin acceso.',
        space_id: spaceId,
      });
    }
    res.status(500).json({
...
async function manejarActualizarCache(req, res) {
  ...
  const { spaceId, token, params } = data;
  try {
    await obtenerTareas(spaceId, token, params);
